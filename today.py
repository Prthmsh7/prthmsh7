...
# ACCESS_TOKEN is the workflow's automatic secrets.GITHUB_TOKEN, so there is no PAT to
# create or rotate. That token only sees public data, so private repositories are excluded
# from the repo, star, commit and LOC counts.
#
# To include private repositories instead, swap the workflow env to a fine-grained PAT with
...
# The cache rebuilds itself automatically when the visible repository count changes.
...
def svg_overwrite(filename, age_data, commit_data, star_data, repo_data, contrib_data, follower_data, loc_data):
    ...
    so a row's total width stays at 5 + len(key) + length characters no matter how the value grows.
    """
...
if __name__ == '__main__':
    ...
    Adapted from Andrew Grant's (Andrew6rant) github-stats README generator
    """
    print('Calculation times:')
    # define global variable for owner ID and calculate the user's account creation date
    # e.g {'id': 'MDQ6VXNlcjU3MzMxMTM0'} and 2022-10-17T07:54:03Z for username 'Prthmsh7'
